...
#  - 評価出力が汎用応答にならないよう、create_evaluation の入力を明示化
# ------------------------------------------------------------
...
# =========================================================
# 評価生成（空入力を避けて明示的に評価指示を与える）
# =========================================================
def create_evaluation() -> str:
    """
    ディクテーション・シャドーイング共通の評価生成。
    空入力だと汎用応答（「お手伝いできます」等）が返るモデルがあるため、
    明示的な評価指示を投入して安定化させる。
    """
    try:
        return st.session_state.chain_evaluation.predict(
            input="上記フォーマットで評価を出力してください。前置きや謝罪は不要。"
        )
    except Exception as e:
